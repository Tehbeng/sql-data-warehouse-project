...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			prd_nm,
			ISNULL(prd_cost, 0) AS prd_cost, -- Replace nulls value to zero
			CASE UPPER(TRIM(prd_line))
				 ...
				 WHEN 'T' THEN 'Touring'
				 ELSE 'n/a'
			END AS prd_line,	-- Map product line codes to a readable format
			CAST(prd_start_dt AS DATE) AS prd_start_dt,
			CAST(
				LEAD(prd_start_dt) OVER(PARTITION BY prd_key ORDER BY prd_start_dt)-1 
				AS DATE) AS prd_end_dt -- Calculate end date to one day before the next	start date
